...
class VisitVideoRecorderController extends GetxController
  ...
  Future<bool> _captureLensHoldFrame() async {
    try {
      ...
      _scheduleLensHoldImageDispose(previous);
      await WidgetsBinding.instance.endOfFrame;
  ...
  void clearLensHoldFrame() {
    ...
    lensHoldOpacity.value = 1.0;
    _scheduleLensHoldImageDispose(image);
  }

  void _scheduleLensHoldImageDispose(ui.Image? image) {
  ...
  Future<void> _applyNeutralExposure(CameraController controller) async {
    ...
    try {
      ...
      await controller.setExposureOffset(0.0.clamp(minOffset, maxOffset));
    } catch (_) {}
  }

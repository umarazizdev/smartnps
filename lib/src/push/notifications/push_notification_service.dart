...
import '../announcements/officer_announcement_coordinator.dart';
import '../announcements/officer_announcement_push.dart';
...
class PushNotificationService {
  ...
  Future<bool> uploadPushToken({bool useSessionCookies = false}) async {
    ...
    final payload = await _buildPushTokenPayload(pushToken: token);
    final uri = Uri.parse(ApiUrls.pushTokenUrl);
  ...
  Options _jsonOptions({bool useSessionCookies = false}) {
    ...
    if (useSessionCookies) {
      ...
      headers['Referer'] = AppRoutes.webBaseUrl;
      headers['Origin'] = AppRoutes.webBaseUrl.replaceAll(RegExp(r'/$'), '');
    }
    return Options(
      headers: headers,
  ...
  static String resolveNotificationUrl(Map<String, dynamic> data) {
    ...
    if (path != null && path.isNotEmpty) {
      ...
      final base = AppRoutes.webBaseUrl.endsWith('/')
          ? AppRoutes.webBaseUrl.substring(0, AppRoutes.webBaseUrl.length - 1)
          : AppRoutes.webBaseUrl;
      return normalizeNotificationUrl('$base$normalizedPath');
    }

    return AppRoutes.defaultPushUrl;
  }

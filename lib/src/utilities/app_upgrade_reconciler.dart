...
import 'secure_storage_access.dart';
...
class AppUpgradeReconciler {
  ...
  static Future<void> reconcileIfNeeded() async {
    ...
    try {
      ...
      final lastSeenBuild = await SecureStorageAccess.read(_kLastSeenBuild);
      final buildChanged =
          lastSeenBuild != null &&
      ...
      if (lastSeenBuild != currentBuild) {
        await SecureStorageAccess.write(_kLastSeenBuild, currentBuild);
      }

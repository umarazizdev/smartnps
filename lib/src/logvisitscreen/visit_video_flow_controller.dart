...
/// Attention-needed note attached to the whole visit batch (not a single media item).
class VisitBatchNote {
  ...
  /// Always included in upload meta.
  /// `needed` is `"yes"` / `"no"` from the Attention needed toggle.
  /// Note fields are filled when needed is yes; otherwise empty.
...
class VisitVideoFlowController extends GetxController {
  ...
  Future<void> _restoreDraft() async {
    _restoring = true;
    try {
      // Prefer last active draft; if empty, fall back to newest pending.
  ...
  /// Clears in-memory draft UI on logout without deleting on-disk drafts.
  ...
  /// Reloads drafts from disk after login (shared device storage).
  ...
  /// Writes the active in-memory draft to disk without clearing UI state.
  ...
  /// Switches in-memory UI to a specific site draft (used by resume dialog).
  ...
  /// Applies site/region from JS bridge `openLogVisit`.
  ///
  /// Loads that site's pending draft if one exists (has captures).
  /// Otherwise opens a fresh in-memory session for the site — no draft is
  /// written until the first photo/video is captured.
  Future<bool> applyBridgePatrolContext(Map<String, dynamic>? payload) async {
    ...
    await _flushPersistQueue();
    // Only persist the previous site if it actually has captures.
    ...
    // Same site already open in memory — keep captures, refresh bridge context.
    if (currentKey == targetKey && mediaItems.isNotEmpty) {
    ...
    activeDraftKey.value = VisitDraftKey.fromContext(merged);
    _applyContextToState(merged);
    // Do not write draft.json until the first capture.
  ...
  /// Builds upload `meta` JSON map from the current draft (API contract).
  ...
  Future<void> _persistDraft() async {
    ...
    _persistQueue = (_persistQueue ?? Future<void>.value()).then((_) async {
      // Empty sessions stay in memory only — no draft folder/file.
  ...
  Future<void> clearAll({bool deleteFiles = true}) async {
    ...
    draftRegionName.value = null;
    patrolContext.value = null;
    // Keep active key pointing at this site folder for the next capture.
    activeDraftKey.value = key;
  }
}

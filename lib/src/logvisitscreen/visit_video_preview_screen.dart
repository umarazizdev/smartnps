...
import 'visit_batch_notes_panel.dart';
...
import 'visit_video_recorder_screen.dart';
...
class VisitVideoPreviewScreen extends GetView<VisitVideoFlowController> {
  static const String routeName = '/visit-video-preview';
  ...
  static String deleteMediaMessage({
    ...
    if (hasNotes) {
      return 'This $media and its note will be removed from your visit media. This cannot be undone.';
    }
    return 'This $media will be removed from your visit media. This cannot be undone.';
  }
  ...
  /// Uploads the active site draft to `POST /api/visits` (multipart + meta).
  ...
  /// Kept for resume-dialog "Submit report" entry point.
...
class _InlineVoiceNotePlayerState extends State<_InlineVoiceNotePlayer> {
  ...
  static final List<double> _waveLevels = List<double>.generate(28, (index) {
    final t = index / 27;
  ...
  @override
  void didUpdateWidget(covariant _InlineVoiceNotePlayer oldWidget) {
  ...
  @override
  Widget build(BuildContext context) {
    final accent = _visitAccentColor(widget.isDark);
    return Row(
      children: [
        Material(
          ...
          child: InkWell(
            onTap: _toggle,
            child: SizedBox.square(
              dimension: widget.compact ? 24 : 26,
              child: Icon(
...
class _InlineVoiceWaveform extends StatelessWidget {
  const _InlineVoiceWaveform({
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      painter: _InlineVoiceWaveformPainter(
        levels: levels,
        color: color,
        progress: progress,
      ),
...
class _InlineVoiceWaveformPainter extends CustomPainter {
  _InlineVoiceWaveformPainter({
    required this.levels,
    required this.color,
    required this.progress,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final barCount = levels.isEmpty ? 20 : levels.length;
    final gap = 1.8;
    ...
    for (var i = 0; i < barCount; i++) {
      ...
      paint.color = isPlayed
          ? color.withValues(alpha: 0.72 + (0.22 * level))
          : color.withValues(alpha: 0.18 + (0.18 * level));
      canvas.drawRRect(
    ...
    if (progress > 0) {
      final knobX = (startX + (totalWidth * progress)).clamp(
        startX,
        startX + totalWidth,
      );
      paint.color = color;
      canvas.drawCircle(Offset(knobX, centerY), 3.4, paint);
      paint.color = Colors.white.withValues(alpha: 0.88);
      canvas.drawCircle(Offset(knobX, centerY), 1.25, paint);
    }
  }

  @override
  bool shouldRepaint(covariant _InlineVoiceWaveformPainter oldDelegate) {

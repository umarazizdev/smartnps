...
class _WaveformPainter extends CustomPainter {
  _WaveformPainter({required this.levels, required this.color, this.progress});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < barCount; i++) {
      ...
      final barHeight = (size.height * level).clamp(5.0, size.height * 0.94);
    ...
    if (playedThrough != null) {
      final knobX = (startX + (totalWidth * playedThrough)).clamp(
        startX,
        startX + totalWidth,
      );
      paint.color = color;
      canvas.drawCircle(Offset(knobX, centerY), 4.2, paint);
      paint.color = Colors.white.withValues(alpha: 0.88);
      canvas.drawCircle(Offset(knobX, centerY), 1.6, paint);
    }
  }

...
class CupertinoTabBarPlatformView: NSObject, FlutterPlatformView, UITabBarDelegate {
  ...
  private var currentSymbolSizes: [CGFloat] = []
  ...
  init(frame: CGRect, viewId: Int64, args: Any?, messenger: FlutterBinaryMessenger) {
    ...
    if let dict = args as? [String: Any] {
      ...
      if let sizes = dict["sfSymbolSizes"] as? [NSNumber] {
        symbolSizes = sizes.map { CGFloat(truncating: $0) }
      }
    ...
    self.currentSymbolSizes = symbolSizes
    ...
    if split && count > rightCount {
      ...
      left.items = makeTabBarItems(in: 0..<leftEnd)
      right.items = makeTabBarItems(in: leftEnd..<count)
      ...
      bar.items = makeTabBarItems(in: 0..<count)
...
channel.setMethodCallHandler { [weak self] call, result in
      ...
      case "setItems":
        if let args = call.arguments as? [String: Any] {
          ...
          if self.isSplit && count > self.rightCountVal, let left = self.tabBarLeft, let right = self.tabBarRight {
            ...
            left.items = self.makeTabBarItems(in: 0..<leftEnd)
            right.items = self.makeTabBarItems(in: leftEnd..<count)
            ...
            bar.items = self.makeTabBarItems(in: 0..<count)
  ...
  private func makeSymbolImage(at index: Int) -> UIImage? {
    guard index < currentSymbols.count else { return nil }
    var image = UIImage(systemName: currentSymbols[index])
    guard let baseImage = image else { return nil }
    if index < currentSymbolSizes.count {
      let pointSize = currentSymbolSizes[index]
      if pointSize > 0 {
        image = baseImage.applyingSymbolConfiguration(
  ...
  private func makeTabBarItems(in range: Range<Int>) -> [UITabBarItem] {
    var items: [UITabBarItem] = []
    for i in range {
      let image = makeSymbolImage(at: i)

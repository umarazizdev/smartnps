...
class StreamHandlerImpl implements EventChannel.StreamHandler {
  ...
  private void disposeListeners(boolean cancelled) {
    ...
    if (foregroundLocationService != null) {
      if (foregroundLocationService.canStopLocationService(cancelled)) {
        foregroundLocationService.stopLocationService();
        foregroundLocationService.disableBackgroundMode();
      } else {
        // Expected with multiple Flutter engines (UI + background service).
        // This engine's locationClient is still stopped below.
        Log.d(
            TAG,
            "Shared Geolocator service kept for other engine/listener; "
                + "stopping this engine's location client");
      }
    }
